...
var init_keycloak = __esm({
  "src/app/services/keycloak.ts"() {
    ...
    KeycloakService = class KeycloakService2 {
      ...
      // ---------- REALMS ----------
      getRealms() {
        return this.http.get(`${this.gw()}/identity/realms`);
      }
      // ---------- PRODUCTS ----------
      createProductWithFile(realm, product, backendZip, frontendZip, frontendBaseUrl) {
        ...
        return this.http.post(`${this.gw()}/identity/${realm}/products`, formData, {
      ...
      getProducts(realm) {
        ...
        if (realm) {
          return this.http.get(`${this.gw()}/identity/products/${realm}`, { headers }).pipe(map((products) => products.map((p) => p.productId || p.id || p.name || p)));
        }
        return this.http.get(`${this.gw()}/gateway/products`, { headers });
      }
      // ---------- LOGIN ----------
      ...
      getUsers(realm) {
        ...
        return this.http.get(`${this.gw()}/identity/users/${realm}`, { headers });
      }
      ...
      saveRoleUrls(realm, clientId, roleName, urls) {
        ...
        return this.http.post(`${this.gw()}/project/roles/save-or-update`, payload, { headers });
      }
...
//# sourceMappingURL=chunk-LZT5VLBC.js.map
