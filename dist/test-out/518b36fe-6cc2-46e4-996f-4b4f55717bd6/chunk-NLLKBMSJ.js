...
function getStoredRealm() {
  if (typeof window === "undefined")
    return null;
  return localStorage.getItem(REALM_KEY);
}
function setStoredRealm(realm) {
  if (typeof window === "undefined")
    return;
  localStorage.setItem(REALM_KEY, realm);
}
function clearStoredRealm() {
  if (typeof window === "undefined")
    return;
...
function setStoredRedirectUrl(redirectUrl) {
  if (typeof window === "undefined")
    return;
...
function getStoredTokenExpiryAt() {
  if (typeof window === "undefined")
    return null;
...
function setStoredTokenExpiryAt(expiresAtMs) {
  if (typeof window === "undefined")
    return;
...
function setStoredTokenExpiryFromToken(token) {
  const expMs = parseTokenExp(token);
  if (expMs) {
    setStoredTokenExpiryAt(expMs);
  }
}
function setStoredTokenExpiryFromExpiresIn(expiresInSeconds) {
  if (typeof expiresInSeconds !== "number" || !Number.isFinite(expiresInSeconds))
    return;
  const expiresAt = Date.now() + Math.max(0, expiresInSeconds) * 1e3;
  setStoredTokenExpiryAt(expiresAt);
}
function isStoredTokenExpired(leewaySeconds = 0) {
  const token = getStoredToken();
  if (!token)
    return true;
  ...
  if (!expiresAt)
    return false;
  const threshold = Date.now() + Math.max(0, leewaySeconds) * 1e3;
...
function touchStoredLastActivity() {
  if (typeof window === "undefined")
    return;
...
function normalizeProductRedirectUrl(rawUrl) {
  if (!rawUrl)
    return null;
  const trimmed = rawUrl.trim();
  if (!trimmed)
...
var TOKEN_KEY, ACCESS_TOKEN_KEY, REFRESH_TOKEN_KEY, REALM_KEY, CLIENT_ID_KEY, REDIRECT_URL_KEY, TOKEN_EXPIRES_AT_KEY, LAST_ACTIVITY_AT_KEY, BASE_URL_KEY;
...
@angular/common/fesm2022/http.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-NLLKBMSJ.js.map
